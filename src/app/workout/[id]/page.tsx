    ...
    <section className="detail-page">
      <div className="detail-layout">
        ...
        <div className="detail-content">
          ...
          <div className="detail-actions"><button className="button button-primary" onClick={() => addToPlan(workout)} disabled={atCapacity && !isPlanned}><span>{isPlanned ? <><Check size={16} /> IN TODAY&apos;S PLAN</> : atCapacity ? "PLAN IS FULL" : <><Plus size={17} /> ADD TO TODAY&apos;S PLAN</>}</span></button><button className="button button-outline" onClick={() => saveWorkout(workout)}>{isSaved ? <><Check size={16} /> SAVED FOR LATER</> : <><BookmarkPlus size={16} /> SAVE FOR LATER</>}</button></div>
